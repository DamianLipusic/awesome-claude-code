  ...
  return (
    <View style={[styles.container, { backgroundColor: bg, borderColor: border }]}>
      ...
      <Svg width={W} height={H}>
        <Defs>
          <LinearGradient id="hydPos" x1="0" y1="0" x2="0" y2="1">
            <Stop offset="0" stopColor={COLORS.accent} stopOpacity="0.4" />
            <Stop offset="1" stopColor={COLORS.accent} stopOpacity="0.02" />
          </LinearGradient>
          <LinearGradient id="hydNeg" x1="0" y1="1" x2="0" y2="0">
        ...
        <Svg x={PAD.left} y={PAD.top} width={CW} height={CH}>
          {yTicks.map(v => (
            <Line key={v} x1={0} y1={toY(v)} x2={CW} y2={toY(v)} stroke={grid} strokeWidth={0.8} />
          ))}
          <Path d={posPath} fill="url(#hydPos)" />
          <Path d={negPath} fill="url(#hydNeg)" />
        ...
        {xTicks.map(t => (
          <SvgText key={t} x={PAD.left + toX(t)} y={H - 4} textAnchor="middle" fontSize={9} fill={muted}>{t}</SvgText>
        ))}
        {yTicks.map(v => (
          <SvgText key={v} x={PAD.left - 6} y={PAD.top + toY(v) + 4} textAnchor="end" fontSize={9} fill={muted}>{v}</SvgText>
        ))}
