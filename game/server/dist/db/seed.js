...
// ─── Config ───────────────────────────────────────────────────
const BETA_STARTING_CASH = 25_000; // Extra cash for beta testers
...
const usedBizNames = new Set();
function randomBizName() {
    for (let i = 0; i < 300; i++) {
        const n = `${pick(BUSINESS_ADJECTIVES)} ${pick(BUSINESS_NOUNS)}`;
        if (!usedBizNames.has(n)) {
            usedBizNames.add(n);
            return n;
        }
    }
...
function salaryForRole(role, efficiency) {
...
// ─── Seed helpers ─────────────────────────────────────────────
async function seedEmployee(client, seasonId, businessId, role) {
    const roles = ['WORKER', 'DRIVER', 'ACCOUNTANT', 'MANAGER', 'SECURITY', 'ENFORCER'];
    ...
    const res = await client.query(`INSERT INTO employees
...
async function seedBusiness(client, ownerId, seasonId, type, city, resourceIds) {
    const tier = 1;
    const capacity = 100;
    ...
    for (const [resName, qty] of inventoryEntries) {
        if (qty > 20 && Math.random() < 0.6) {
            ...
            const price = parseFloat((resDef.base_value * constants_1.AI_MARKUP * priceVariance).toFixed(2));
         ...
         VALUES ($1,'PLAYER_SELL',$2,$3,$4,$5,$6,$6,$7,1,
                 NOW() + INTERVAL '48 hours',$8,'OPEN')`, [seasonId, ownerId, bizId, resId, city, listQty, price, Math.random() < 0.3]);
        }
    }
    return bizId;
}
...
async function main() {
    ...
    await (0, client_1.withTransaction)(async (client) => {
        ...
        const seasonRes = await client.query(`INSERT INTO season_profiles
       ...
       RETURNING id`, [
        ...
        for (const res of SEED_RESOURCES) {
            ...
            const aiPrice = parseFloat((res.base_value * constants_1.AI_MARKUP).toFixed(2));
            const rRes = await client.query(`INSERT INTO resources
         ...
         RETURNING id`, [res.name, res.category, res.tier, res.base_value, res.weight,
        ...
        const existing = await client.query(`SELECT id, name FROM resources WHERE season_id = $1`, [seasonId]);
        ...
        for (const res of SEED_RESOURCES) {
            ...
            if (!resId)
                continue;
            let price = res.base_value * constants_1.AI_MARKUP;
            for (let day = PRICE_HISTORY_DAYS; day >= 0; day--) {
                ...
                for (let tick = 0; tick < 8; tick++) {
                    ...
                    await client.query(`INSERT INTO price_history (resource_id, season_id, price, recorded_at)
             VALUES ($1,$2,$3, NOW() - INTERVAL '${hoursAgo} hours')`, [resId, seasonId, parseFloat(price.toFixed(2))]);
                }
            }
        }
        ...
        for (const city of entities_1.CITIES) {
            for (const [resName, resId] of Object.entries(resourceIds)) {
                ...
                if (!resDef)
                    continue;
                const tier = resDef.tier;
                const cap = constants_1.AI_QUANTITY_CAPS[city.size]?.[tier] ?? 500;
                const aiPrice = parseFloat((resDef.base_value * constants_1.AI_MARKUP).toFixed(2));
                const aiBuyPrice = parseFloat((resDef.base_value * constants_1.AI_BUY_DISCOUNT).toFixed(2));
                await client.query(`INSERT INTO market_listings
             (season_id, listing_type, seller_id, business_id, resource_id,
              city, quantity, quantity_remaining, price_per_unit, min_quantity,
              expires_at, is_anonymous, status)
           VALUES ($1,'AI_SELL',NULL,NULL,$2,$3,$4,$4,$5,1,NOW()+INTERVAL '2 hours',false,'OPEN')`, [seasonId, resId, city.name, cap, aiPrice]);
                await client.query(`INSERT INTO market_listings
             (season_id, listing_type, seller_id, business_id, resource_id,
              city, quantity, quantity_remaining, price_per_unit, min_quantity,
              is_anonymous, status)
           VALUES ($1,'AI_BUY',NULL,NULL,$2,$3,$4,$4,$5,1,false,'OPEN')`, [seasonId, resId, city.name, cap, aiBuyPrice]);
            }
        }
        console.log(`    ✓ ${entities_1.CITIES.length * SEED_RESOURCES.length * 2} AI listings created`);
        // ── 4. NPC players ────────────────────────────────────────
        console.log(`[4/6] Creating ${NPC_COUNT} NPC players with businesses...`);
        const npcPassword = await bcrypt_1.default.hash('npc-internal-account-not-for-login', 10);
        for (const npc of NPC_PROFILES) {
            // Create player
            const pRes = await client.query(`INSERT INTO players
         ...
         RETURNING id`, [
            ...
            const heat = npc.alignment === 'CRIMINAL'
                ...
                : npc.alignment === 'MIXED'
                    ? randInt(0, 150)
                    : 0;
            const heatLevel = heat >= 600 ? 'BURNING' : heat >= 300 ? 'HOT' : heat >= 100 ? 'WARM' : 'COLD';
            await client.query(`INSERT INTO heat_scores (player_id, season_id, score, level, last_criminal_act, decay_rate)
         ...
         ON CONFLICT (player_id, season_id) DO NOTHING`, [
           ...
           VALUES ($1,$2,$3,$4,$5)
           ON CONFLICT (player_id, season_id) DO NOTHING`, [npcId, seasonId, dirty, dirty * rand(1.2, 2.0), dirty * rand(0.1, 0.5)]);
            }
            // Create businesses
        ...
        const roleOptions = ['WORKER', 'DRIVER', 'ACCOUNTANT', 'MANAGER', 'SECURITY', 'ENFORCER'];
       ...
       WHERE season_id = $1 ORDER BY net_worth DESC LIMIT 10`, [seasonId]);
        const topJson = JSON.stringify(topPlayers.rows.map((p, i) => ({
            rank: i + 1,
            player_id: p.id,
            username: p.username,
       ...
       WHERE id = $1`, [seasonId, topJson]);
        ...
        const counts = await client.query(`SELECT
         ...
         (SELECT COUNT(*) FROM market_listings WHERE season_id = $1)::text AS listings`, [seasonId]);
