...
exports.recalculateNetWorth = recalculateNetWorth;
const client_1 = require("../db/client");
/**
 * Recalculates and persists net_worth for a player.
 * net_worth = cash + business_value (depreciated) + inventory_value
 */
async function recalculateNetWorth(playerId) {
    // Get current cash
    const playerRes = await (0, client_1.query)('SELECT cash FROM players WHERE id = $1', [playerId]);
    if (playerRes.rows.length === 0)
        return 0;
    const cash = Number(playerRes.rows[0].cash);
    // Business value: depreciated startup cost per type/tier
    const businessRes = await (0, client_1.query)(`SELECT type, tier FROM businesses WHERE owner_id = $1 AND status != 'BANKRUPT'`, [playerId]);
    const STARTUP_COSTS = {
    ...
    const inventoryRes = await (0, client_1.query)(`SELECT b.inventory, s.id as season_id
     FROM businesses b
     JOIN players p ON b.owner_id = p.id
     JOIN season_profiles s ON s.id = p.season_id
     WHERE b.owner_id = $1 AND b.status != 'BANKRUPT'`, [playerId]);
    ...
    for (const row of inventoryRes.rows) {
        ...
        for (const [resourceId, qty] of Object.entries(inventory)) {
            if (qty <= 0)
                continue;
            const priceRes = await (0, client_1.query)('SELECT current_ai_price FROM resources WHERE id = $1', [resourceId]);
    ...
    await (0, client_1.query)('UPDATE players SET net_worth = $1 WHERE id = $2', [netWorth, playerId]);
    return netWorth;
}
