 ...
 * arrives. The player has 80 seconds to decide.
 *
 * Choices:
 ...
 *   nextSpawnTick: tick,
 *   totalVisits: number, totalCommissioned: number, totalListened: number,
 * }
 */
...
export const COMMISSION_FOOD_RATE       = 0.1;
...
export function initWanderingBard() {
  if (!state.wanderingBard) {
    state.wanderingBard = { active: null, nextSpawnTick: _nextSpawnTick(), totalVisits: 0, totalCommissioned: 0, totalListened: 0 };
  }
...
export function wanderingBardTick() {
  ...
  if (b.active) {
    if (state.tick >= b.active.expiresAt) {
      b.active        = null; b.nextSpawnTick = _nextSpawnTick();
...
export function getActiveWanderingBard() { return state.wanderingBard?.active ?? null; }
...
export function commissionBardPerformance() {
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({ id: 'bardCommission', resource: 'food', rateMult: 1 + (COMMISSION_FOOD_RATE / Math.max(0.001, (state.rates?.food ?? 1))), expiresAt: state.tick + COMMISSION_DURATION_TICKS });
  }
  b.active            = null; b.nextSpawnTick     = _nextSpawnTick(); b.totalCommissioned = (b.totalCommissioned ?? 0) + 1;
  emit(Events.WANDERING_BARD_CHANGED, { commissioned: true }); emit(Events.RESOURCE_CHANGED, {});
...
export function listenToBardStories() {
  ...
  b.active        = null; b.nextSpawnTick = _nextSpawnTick(); b.totalListened = (b.totalListened ?? 0) + 1;
