 ...
 * one of three resource gift options.
 *
 * Tribute options:
 ...
 *   nextSpawnTick: tick, totalCaravans: number, totalClaimed: number,
 * }
 */
...
const WINDOW_TICKS     = 90 * TICKS_PER_SECOND;
const MIN_AGE          = 1;
const MIN_PLAYER_TILES = 10;

export function initTributeCaravan() {
  if (!state.tributeCaravan) {
    state.tributeCaravan = { active: null, nextSpawnTick: _nextSpawnTick(), totalCaravans: 0, totalClaimed: 0 };
  }
...
export function tributeCaravanTick() {
  ...
  if (tc.active) {
    if (state.tick >= tc.active.expiresAt) {
      tc.active        = null; tc.nextSpawnTick = _nextSpawnTick();
  ...
  tc.active = { options: _buildOptions(), expiresAt: state.tick + WINDOW_TICKS };
...
export function getActiveTributeCaravan() { return state.tributeCaravan?.active ?? null; }
...
export function claimTribute(idx) {
  ...
  tc.active        = null; tc.nextSpawnTick = _nextSpawnTick(); tc.totalClaimed  = (tc.totalClaimed ?? 0) + 1;
  emit(Events.TRIBUTE_CARAVAN_CHANGED, { claimed: true }); emit(Events.RESOURCE_CHANGED, {});
  const rewardStr = Object.entries(option.reward).map(([r, a]) => `+${a} ${r}`).join(', ');
...
function _buildOptions() {
  const gold = _ri(80, 120); const food = _ri(120, 150);
  return [
    { id: 'gold_tribute',   icon: '💰', title: 'Gold Tribute',   desc: `Merchants bearing ${gold} gold coins from the outer settlements.`,          reward: { gold } },
    { id: 'food_tribute',   icon: '🌾', title: 'Food Tribute',   desc: `Farmers delivering ${food} bushels of grain and preserved provisions.`,     reward: { food } },
    { id: 'supply_tribute', icon: '📦', title: 'Supply Tribute', desc: 'Labourers hauling a mixed cargo of building materials from village stockpiles.', reward: { wood: 50, stone: 40, iron: 30 } },
  ];
}