 ...
 * jubilee is declared. The player has 85 seconds to decide.
 *
 * Choices:
...
const SPAWN_MIN        = 18 * 60 * TICKS_PER_SECOND;
...
export const PARADE_FOOD_RATE        = 0.15;
export const PARADE_GOLD_RATE        = 0.10;
export const PARADE_DURATION_TICKS   = 3 * 60 * TICKS_PER_SECOND;
...
export function initImperialJubilee() {
  if (!state.imperialJubilee) {
    state.imperialJubilee = { active: null, nextSpawnTick: _nextSpawnTick(), totalJubilees: 0, totalParades: 0, totalFeasts: 0 };
  }
...
export function imperialJubileeTick() {
  ...
  if (j.active) {
    if (state.tick >= j.active.expiresAt) {
      j.active        = null; j.nextSpawnTick = _nextSpawnTick();
...
export function getActiveImperialJubilee() { return state.imperialJubilee?.active ?? null; }
...
export function declareGrandParade() {
  ...
  if ((state.resources.gold ?? 0) < PARADE_GOLD_COST) return { ok: false, reason: `Need ${PARADE_GOLD_COST} gold.` };
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers = state.randomEvents.activeModifiers.filter(m => m.id !== 'jubileeParadeFood' && m.id !== 'jubileeParadeGold');
    state.randomEvents.activeModifiers.push({ id: 'jubileeParadeFood', resource: 'food', rateMult: 1 + (PARADE_FOOD_RATE / Math.max(0.001, (state.rates?.food ?? 1))), expiresAt: state.tick + PARADE_DURATION_TICKS });
    state.randomEvents.activeModifiers.push({ id: 'jubileeParadeGold', resource: 'gold', rateMult: 1 + (PARADE_GOLD_RATE / Math.max(0.001, (state.rates?.gold ?? 1))), expiresAt: state.tick + PARADE_DURATION_TICKS });
  }
  j.active        = null; j.nextSpawnTick = _nextSpawnTick(); j.totalParades  = (j.totalParades ?? 0) + 1;
  emit(Events.IMPERIAL_JUBILEE_CHANGED, { parade: true }); emit(Events.RESOURCE_CHANGED, {});
...
export function declareRoyalFeast() {
  ...
  if (!j?.active) return { ok: false, reason: 'No jubilee declared.' };
  if (state.tick >= j.active.expiresAt) return { ok: false, reason: 'The jubilee has passed.' };
  if ((state.resources.food ?? 0) < FEAST_FOOD_COST) return { ok: false, reason: `Need ${FEAST_FOOD_COST} food.` };
  ...
  j.active        = null; j.nextSpawnTick = _nextSpawnTick(); j.totalFeasts = (j.totalFeasts ?? 0) + 1;
  emit(Events.IMPERIAL_JUBILEE_CHANGED, { feast: true }); emit(Events.RESOURCE_CHANGED, {});
