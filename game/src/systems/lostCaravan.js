...
export const SHELTER_GOLD_RATE       = 0.15;
export const SHELTER_DURATION_TICKS  = 3 * 60 * TICKS_PER_SECOND;
...
export function initLostCaravan() {
  if (!state.lostCaravan) {
    state.lostCaravan = { active: null, nextSpawnTick: _nextSpawnTick(), totalEncounters: 0, totalSheltered: 0, totalHired: 0, totalTurnedAway: 0 };
  }
...
export function lostCaravanTick() {
  ...
  if (lc.active) {
    if (state.tick >= lc.active.expiresAt) {
      lc.active          = null; lc.nextSpawnTick = _nextSpawnTick(); lc.totalTurnedAway = (lc.totalTurnedAway ?? 0) + 1;
...
export function getActiveLostCaravan() { return state.lostCaravan?.active ?? null; }
...
export function offerCaravanShelter() {
  ...
  if ((state.resources.food ?? 0) < SHELTER_FOOD_COST) return { ok: false, reason: `Need ${SHELTER_FOOD_COST} food.` };
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({ id: 'caravanShelter', resource: 'gold', rateMult: 1 + (SHELTER_GOLD_RATE / Math.max(0.001, Math.abs(state.rates?.gold ?? 1))), expiresAt: state.tick + SHELTER_DURATION_TICKS });
  }
  lc.active         = null; lc.nextSpawnTick  = _nextSpawnTick(); lc.totalSheltered = (lc.totalSheltered ?? 0) + 1;
  emit(Events.LOST_CARAVAN_CHANGED, { sheltered: true }); emit(Events.RESOURCE_CHANGED, {});
...
export function hireCaravanGuides() {
  ...
  lc.active        = null; lc.nextSpawnTick = _nextSpawnTick(); lc.totalHired    = (lc.totalHired ?? 0) + 1;
  emit(Events.LOST_CARAVAN_CHANGED, { hired: true }); emit(Events.RESOURCE_CHANGED, {});
