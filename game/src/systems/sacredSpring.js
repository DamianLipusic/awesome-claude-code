 ...
 *   💰 Sell Water Rights — −8 prestige  → +100 gold (commercial exploitation)
...
export function sacredSpringTick() {
  ...
  if (ss.active) {
    if (state.tick >= ss.active.expiresAt) {
      ss.active        = null;
      ss.nextSpawnTick = _nextSpawnTick();
...
export function getActiveSacredSpring() {
  return state.sacredSpring?.active ?? null;
}
...
/**
 * Bless the Waters — pay 20 mana, receive +35 food, +12 morale, +10 prestige.
 */
export function blessSpringWaters() {
  ...
  if ((state.resources.mana ?? 0) < BLESS_MANA_COST) {
    return { ok: false, reason: `Need ${BLESS_MANA_COST} mana.` };
  }
  ...
  ss.active       = null;
  ss.nextSpawnTick = _nextSpawnTick();
  ss.totalBlessed = (ss.totalBlessed ?? 0) + 1;

  emit(Events.SACRED_SPRING_CHANGED, { blessed: true });
...
/**
 * Sell Water Rights — lose 8 prestige, gain +100 gold.
 */
export function sellSpringWaterRights() {
  ...
  ss.active       = null;
  ss.nextSpawnTick = _nextSpawnTick();
  ss.totalSold    = (ss.totalSold ?? 0) + 1;

  emit(Events.SACRED_SPRING_CHANGED, { sold: true });
  emit(Events.RESOURCE_CHANGED, {});
...
/**
 * Protect the Spring — free, gain +6 morale and +4 prestige.
 */
export function protectSacredSpring() {
  ...
  ss.active       = null;
  ss.nextSpawnTick = _nextSpawnTick();
  ss.totalProtected = (ss.totalProtected ?? 0) + 1;
