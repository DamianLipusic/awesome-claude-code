 ...
 *
 * Choices:
 *   🧘 Seek Counsel  — free → +10 prestige, +0.1 mana/s for 2 min (arcane insight)
...
export const COUNSEL_MANA_RATE        = 0.1;  // +0.1 mana/s arcane insight
export const COUNSEL_DURATION_TICKS   = 2 * 60 * TICKS_PER_SECOND; // 2 min
...
export function mountainHermitTick() {
  ...
  if (h.active) {
    if (state.tick >= h.active.expiresAt) {
      h.active        = null;
      h.nextSpawnTick = _nextSpawnTick();
...
export function getActiveMountainHermit() {
  return state.mountainHermit?.active ?? null;
}
...
/**
 * Seek Counsel — free, gain +10 prestige and +0.1 mana/s for 2 minutes.
 */
export function seekHermitCounsel() {
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'hermitCounsel',
      resource:  'mana',
      rateMult:  1 + (COUNSEL_MANA_RATE / Math.max(0.001, (state.rates?.mana ?? 1))),
      expiresAt: state.tick + COUNSEL_DURATION_TICKS,
    });
  }

  h.active         = null;
  h.nextSpawnTick  = _nextSpawnTick();
  h.totalCounseled = (h.totalCounseled ?? 0) + 1;

  emit(Events.MOUNTAIN_HERMIT_CHANGED, { counseled: true });
...
/**
 * Offer Tribute — pay 25 gold, receive +25 prestige and +12 morale.
 */
export function offerHermitTribute() {
  ...
  if ((state.resources.gold ?? 0) < TRIBUTE_GOLD_COST) {
    return { ok: false, reason: `Need ${TRIBUTE_GOLD_COST} gold.` };
  }
  ...
  h.active        = null;
  h.nextSpawnTick = _nextSpawnTick();
  h.totalTributed = (h.totalTributed ?? 0) + 1;

  emit(Events.MOUNTAIN_HERMIT_CHANGED, { tributed: true });
  emit(Events.RESOURCE_CHANGED, {});
...
/**
 * Leave in Peace — no cost, no reward.
 */
