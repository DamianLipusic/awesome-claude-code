...
export const CHART_MANA_RATE        = 0.3; // +0.3 mana/s
export const CHART_DURATION_TICKS   = Math.round(2.5 * 60 * TICKS_PER_SECOND); // 2.5 min
...
export function travelingAstrologerTick() {
  ...
  if (ta.active) {
    if (state.tick >= ta.active.expiresAt) {
      ta.active        = null;
      ta.nextSpawnTick = _nextSpawnTick();
...
export function getActiveTravelingAstrologer() {
  return state.travelingAstrologer?.active ?? null;
}
...
export function commissionStarChart() {
  ...
  if ((state.resources.gold ?? 0) < CHART_GOLD_COST) {
    return { ok: false, reason: `Need ${CHART_GOLD_COST} gold.` };
  }
  if ((state.resources.mana ?? 0) < CHART_MANA_COST) {
    return { ok: false, reason: `Need ${CHART_MANA_COST} mana.` };
  }

  state.resources.gold -= CHART_GOLD_COST;
  state.resources.mana -= CHART_MANA_COST;
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'astrologerChart',
      resource:  'mana',
      rateMult:  1 + (CHART_MANA_RATE / Math.max(0.001, Math.abs(state.rates?.mana ?? 1))),
      expiresAt: state.tick + CHART_DURATION_TICKS,
    });
  }

  ta.active            = null;
  ta.nextSpawnTick     = _nextSpawnTick();
  ta.totalCommissioned = (ta.totalCommissioned ?? 0) + 1;

  emit(Events.ASTROLOGER_CHANGED, { commissioned: true });
...
export function seekCelestialWisdom() {
  ...
  if (!ta?.active) return { ok: false, reason: 'No astrologer present.' };
  if (state.tick >= ta.active.expiresAt) return { ok: false, reason: 'The astrologer has departed.' };
  if ((state.resources.mana ?? 0) < WISDOM_MANA_COST) {
    return { ok: false, reason: `Need ${WISDOM_MANA_COST} mana.` };
  }
  ...
  ta.active         = null;
  ta.nextSpawnTick  = _nextSpawnTick();
  ta.totalConsulted = (ta.totalConsulted ?? 0) + 1;

  emit(Events.ASTROLOGER_CHANGED, { consulted: true });
  emit(Events.RESOURCE_CHANGED, {});
...
export function offerAstrologerRefreshments() {
  ...
  if (!ta?.active) return { ok: false, reason: 'No astrologer present.' };
  if (state.tick >= ta.active.expiresAt) return { ok: false, reason: 'The astrologer has departed.' };
  if ((state.resources.food ?? 0) < REFRESH_FOOD_COST) {
    return { ok: false, reason: `Need ${REFRESH_FOOD_COST} food.` };
  }
  ...
  ta.active         = null;
  ta.nextSpawnTick  = _nextSpawnTick();
  ta.totalRefreshed = (ta.totalRefreshed ?? 0) + 1;

  emit(Events.ASTROLOGER_CHANGED, { refreshed: true });
  emit(Events.RESOURCE_CHANGED, {});
