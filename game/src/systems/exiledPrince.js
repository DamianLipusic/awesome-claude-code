...
export const ASYLUM_GOLD_RATE        = 0.12; // +0.12 gold/s from prince's treasury
export const ASYLUM_DURATION_TICKS   = 3 * 60 * TICKS_PER_SECOND; // 3 min

export const ADVISOR_GOLD_COST       = 40;
export const ADVISOR_PRESTIGE_REWARD = 30;
export const ADVISOR_MORALE_REWARD   = 15;
export const ADVISOR_IRON_RATE       = 0.1; // +0.1 iron/s military insight
export const ADVISOR_DURATION_TICKS  = 2 * 60 * TICKS_PER_SECOND; // 2 min
...
export function getActiveExiledPrince() {
  return state.exiledPrince?.active ?? null;
}
...
/**
 * Grant Asylum — free; +12 morale, +10 prestige, +0.12 gold/s for 3 min.
 */
export function grantAsylum() {
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'exiledPrinceAsylum',
      resource:  'gold',
      rateMult:  1 + (ASYLUM_GOLD_RATE / Math.max(0.001, Math.abs(state.rates?.gold ?? 1))),
      expiresAt: state.tick + ASYLUM_DURATION_TICKS,
    });
  }

  ep.active       = null;
  ep.nextSpawnTick = _nextSpawnTick();
  ep.totalGranted = (ep.totalGranted ?? 0) + 1;

  emit(Events.EXILED_PRINCE_CHANGED, { granted: true });
...
/**
 * Accept as Advisor — pay 40 gold; +30 prestige, +15 morale, +0.1 iron/s for 2 min.
 */
export function acceptPrinceAdvisor() {
  const ep = state.exiledPrince;
  if (!ep?.active) return { ok: false, reason: 'No prince present.' };
  if (state.tick >= ep.active.expiresAt) return { ok: false, reason: 'The prince has departed.' };
  if ((state.resources.gold ?? 0) < ADVISOR_GOLD_COST) {
    return { ok: false, reason: `Need ${ADVISOR_GOLD_COST} gold.` };
  }
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'exiledPrinceAdvisor',
      resource:  'iron',
      rateMult:  1 + (ADVISOR_IRON_RATE / Math.max(0.001, Math.abs(state.rates?.iron ?? 1))),
      expiresAt: state.tick + ADVISOR_DURATION_TICKS,
    });
  }

  ep.active        = null;
  ep.nextSpawnTick = _nextSpawnTick();
  ep.totalAdvisors = (ep.totalAdvisors ?? 0) + 1;

  emit(Events.EXILED_PRINCE_CHANGED, { advisor: true });
  emit(Events.RESOURCE_CHANGED, {});
