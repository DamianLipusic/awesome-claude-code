 ...
 * blacksmith arrives at the imperial forge district seeking work. The player
 * has 75 seconds to decide how to engage them.
 *
 * Choices:
 *   ⚔️ Commission Elite Weapons — pay 40 iron → +0.2 iron/s for 2 min + 20 prestige
 *                                  (blacksmith reveals advanced forging techniques)
 *   🛡️ Forge Iron Tools         — pay 20 iron + 15 gold → +20 iron lump sum + 15 prestige
 *                                  (practical tool production for the empire)
 *   🤝 Offer Lodging            — pay 10 food → +6 morale + 5 prestige
 *                                  (the blacksmith departs with warm regards)
 *
...
const SPAWN_MIN        = 11 * 60 * TICKS_PER_SECOND; // 11 min
...
export const WEAPONS_IRON_RATE       = 0.2; // +0.2 iron/s
export const WEAPONS_DURATION_TICKS  = 2 * 60 * TICKS_PER_SECOND; // 2 min
...
export function wanderingBlacksmithTick() {
  ...
  if (wb.active) {
    if (state.tick >= wb.active.expiresAt) {
      wb.active        = null;
      wb.nextSpawnTick = _nextSpawnTick();
...
export function getActiveWanderingBlacksmith() {
  return state.wanderingBlacksmith?.active ?? null;
}
...
/**
 * Commission Elite Weapons — pay 40 iron; +0.2 iron/s for 2 min + 20 prestige.
 */
export function commissionEliteWeapons() {
  ...
  if ((state.resources.iron ?? 0) < WEAPONS_IRON_COST) {
    return { ok: false, reason: `Need ${WEAPONS_IRON_COST} iron.` };
  }
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'blacksmithWeapons',
      resource:  'iron',
      rateMult:  1 + (WEAPONS_IRON_RATE / Math.max(0.001, Math.abs(state.rates?.iron ?? 1))),
      expiresAt: state.tick + WEAPONS_DURATION_TICKS,
    });
  }

  wb.active           = null;
  wb.nextSpawnTick    = _nextSpawnTick();
  wb.totalCommissions = (wb.totalCommissions ?? 0) + 1;

  emit(Events.BLACKSMITH_CHANGED, { commissioned: true });
...
/**
 * Forge Iron Tools — pay 20 iron + 15 gold; +20 iron lump sum + 15 prestige.
 */
export function forgeIronTools() {
  const wb = state.wanderingBlacksmith;
  if (!wb?.active) return { ok: false, reason: 'No blacksmith present.' };
  if (state.tick >= wb.active.expiresAt) return { ok: false, reason: 'The blacksmith has moved on.' };
  if ((state.resources.iron ?? 0) < TOOLS_IRON_COST) {
    return { ok: false, reason: `Need ${TOOLS_IRON_COST} iron.` };
  }
  if ((state.resources.gold ?? 0) < TOOLS_GOLD_COST) {
    return { ok: false, reason: `Need ${TOOLS_GOLD_COST} gold.` };
  }

  state.resources.iron -= TOOLS_IRON_COST;
  state.resources.gold -= TOOLS_GOLD_COST;
  ...
  wb.active        = null;
  wb.nextSpawnTick = _nextSpawnTick();
  wb.totalForged   = (wb.totalForged ?? 0) + 1;

  emit(Events.BLACKSMITH_CHANGED, { forged: true });
  emit(Events.RESOURCE_CHANGED, {});
...
export function offerBlacksmithLodging() {
  ...
  if (!wb?.active) return { ok: false, reason: 'No blacksmith present.' };
  if (state.tick >= wb.active.expiresAt) return { ok: false, reason: 'The blacksmith has moved on.' };
  if ((state.resources.food ?? 0) < LODGING_FOOD_COST) {
    return { ok: false, reason: `Need ${LODGING_FOOD_COST} food.` };
  }
  ...
  wb.active        = null;
  wb.nextSpawnTick = _nextSpawnTick();
  wb.totalLodged   = (wb.totalLodged ?? 0) + 1;

  emit(Events.BLACKSMITH_CHANGED, { lodged: true });
  emit(Events.RESOURCE_CHANGED, {});
