 ...
 *
 * Choices:
 *   🔨 Hire the Artisan   — pay 70 gold → +60 prestige, +0.15 iron/s for 3 min (crafting output)
...
export const HIRE_IRON_RATE        = 0.15; // +0.15 iron/s crafting output
export const HIRE_DURATION_TICKS   = 3 * 60 * TICKS_PER_SECOND; // 3 min
...
export function masterArtisanTick() {
  ...
  if (a.active) {
    if (state.tick >= a.active.expiresAt) {
      a.active        = null;
      a.nextSpawnTick = _nextSpawnTick();
...
export function getActiveMasterArtisan() {
  return state.masterArtisan?.active ?? null;
}
...
/**
 * Hire the Artisan — pay 70 gold, receive +60 prestige and +0.15 iron/s for 3 minutes.
 */
export function hireMasterArtisan() {
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'masterArtisanHire',
      resource:  'iron',
      rateMult:  1 + (HIRE_IRON_RATE / Math.max(0.001, (state.rates?.iron ?? 1))),
      expiresAt: state.tick + HIRE_DURATION_TICKS,
    });
  }

  a.active        = null;
  a.nextSpawnTick = _nextSpawnTick();
  a.totalHired    = (a.totalHired ?? 0) + 1;

  emit(Events.MASTER_ARTISAN_CHANGED, { hired: true });
...
/**
 * Commission Pieces — pay 40 wood + 20 stone, receive +80 gold and +30 prestige.
 */
export function commissionArtisanPieces() {
  ...
  if ((state.resources.wood ?? 0) < COMMISSION_WOOD_COST) {
    return { ok: false, reason: `Need ${COMMISSION_WOOD_COST} wood.` };
  }
  if ((state.resources.stone ?? 0) < COMMISSION_STONE_COST) {
    return { ok: false, reason: `Need ${COMMISSION_STONE_COST} stone.` };
  }
  ...
  a.active            = null;
  a.nextSpawnTick     = _nextSpawnTick();
  a.totalCommissioned = (a.totalCommissioned ?? 0) + 1;

  emit(Events.MASTER_ARTISAN_CHANGED, { commissioned: true });
  emit(Events.RESOURCE_CHANGED, {});
...
/**
 * Bid Farewell — no cost, no reward.
 */
