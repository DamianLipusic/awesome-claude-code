 ...
 * prince passes through the empire seeking profitable arrangements. The player
 * has 75 seconds to decide how to engage them.
 *
 * Choices:
 *   💰 Arrange Trade Deal     — pay 50 gold → +0.35 gold/s for 2.5 min + 22 prestige
 *                                (an exclusive commerce arrangement enriches both parties)
 *   🤝 Exchange Valuable Goods — pay 20 food + 10 wood → +45 gold lump sum + 15 prestige
 *                                (a straightforward goods-for-gold exchange)
 *   🍷 Host Lavish Reception   — pay 30 food → +8 morale + 8 prestige
 *                                (a grand reception impresses the merchant prince)
 *
...
export const DEAL_GOLD_RATE         = 0.35; // +0.35 gold/s
export const DEAL_DURATION_TICKS    = Math.round(2.5 * 60 * TICKS_PER_SECOND); // 2.5 min
...
export function merchantPrinceTick() {
  ...
  if (mp.active) {
    if (state.tick >= mp.active.expiresAt) {
      mp.active        = null;
      mp.nextSpawnTick = _nextSpawnTick();
...
export function getActiveMerchantPrince() {
  return state.merchantPrince?.active ?? null;
}
...
export function arrangeTradeDeal() {
  ...
  if ((state.resources.gold ?? 0) < DEAL_GOLD_COST) {
    return { ok: false, reason: `Need ${DEAL_GOLD_COST} gold.` };
  }
  ...
  mp.active        = null;
  mp.nextSpawnTick = _nextSpawnTick();
  mp.totalDeals    = (mp.totalDeals ?? 0) + 1;

  emit(Events.MERCHANT_PRINCE_CHANGED, { deal: true });
...
export function exchangeValuableGoods() {
  ...
  if (!mp?.active) return { ok: false, reason: 'No merchant prince present.' };
  if (state.tick >= mp.active.expiresAt) return { ok: false, reason: 'The merchant prince has departed.' };
  if ((state.resources.food ?? 0) < EXCHANGE_FOOD_COST) {
    return { ok: false, reason: `Need ${EXCHANGE_FOOD_COST} food.` };
  }
  if ((state.resources.wood ?? 0) < EXCHANGE_WOOD_COST) {
    return { ok: false, reason: `Need ${EXCHANGE_WOOD_COST} wood.` };
  }
  ...
  mp.active          = null;
  mp.nextSpawnTick   = _nextSpawnTick();
  mp.totalExchanges  = (mp.totalExchanges ?? 0) + 1;

  emit(Events.MERCHANT_PRINCE_CHANGED, { exchanged: true });
  emit(Events.RESOURCE_CHANGED, {});
...
export function hostMerchantReception() {
  ...
  if (!mp?.active) return { ok: false, reason: 'No merchant prince present.' };
  if (state.tick >= mp.active.expiresAt) return { ok: false, reason: 'The merchant prince has departed.' };
  if ((state.resources.food ?? 0) < RECEPTION_FOOD_COST) {
    return { ok: false, reason: `Need ${RECEPTION_FOOD_COST} food.` };
  }
  ...
  mp.active           = null;
  mp.nextSpawnTick    = _nextSpawnTick();
  mp.totalReceptions  = (mp.totalReceptions ?? 0) + 1;

  emit(Events.MERCHANT_PRINCE_CHANGED, { reception: true });
  emit(Events.RESOURCE_CHANGED, {});
