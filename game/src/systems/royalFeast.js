 ...
 * announces a Royal Feast to celebrate the empire's prosperity. The player
 * has 85 seconds to decide how to host it.
 *
 * Choices:
...
export const BANQUET_FOOD_RATE       = 0.15; // +0.15 food/s
export const BANQUET_DURATION_TICKS  = 3 * 60 * TICKS_PER_SECOND; // 3 min
...
export function royalFeastTick() {
  ...
  if (rf.active) {
    if (state.tick >= rf.active.expiresAt) {
      rf.active        = null;
      rf.nextSpawnTick = _nextSpawnTick();
...
export function getActiveRoyalFeast() {
  return state.royalFeast?.active ?? null;
}
...
export function declareGrandBanquet() {
  ...
  if ((state.resources.food ?? 0) < BANQUET_FOOD_COST) {
    return { ok: false, reason: `Need ${BANQUET_FOOD_COST} food.` };
  }
  if ((state.resources.gold ?? 0) < BANQUET_GOLD_COST) {
    return { ok: false, reason: `Need ${BANQUET_GOLD_COST} gold.` };
  }

  state.resources.food -= BANQUET_FOOD_COST;
  state.resources.gold -= BANQUET_GOLD_COST;
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'royalFeastBanquet',
      resource:  'food',
      rateMult:  1 + (BANQUET_FOOD_RATE / Math.max(0.001, Math.abs(state.rates?.food ?? 1))),
      expiresAt: state.tick + BANQUET_DURATION_TICKS,
    });
  }

  rf.active      = null;
  rf.nextSpawnTick = _nextSpawnTick();
  rf.totalGrand  = (rf.totalGrand ?? 0) + 1;

  emit(Events.ROYAL_FEAST_CHANGED, { grand: true });
...
export function declareModestCelebration() {
  ...
  if (!rf?.active) return { ok: false, reason: 'No feast pending.' };
  if (state.tick >= rf.active.expiresAt) return { ok: false, reason: 'The feast opportunity has passed.' };
  if ((state.resources.food ?? 0) < MODEST_FOOD_COST) {
    return { ok: false, reason: `Need ${MODEST_FOOD_COST} food.` };
  }
  ...
  rf.active        = null;
  rf.nextSpawnTick = _nextSpawnTick();
  rf.totalModest   = (rf.totalModest ?? 0) + 1;

  emit(Events.ROYAL_FEAST_CHANGED, { modest: true });
  emit(Events.RESOURCE_CHANGED, {});
...
export function declareHoliday() {
  ...
  rf.active        = null;
  rf.nextSpawnTick = _nextSpawnTick();
  rf.totalHoliday  = (rf.totalHoliday ?? 0) + 1;
