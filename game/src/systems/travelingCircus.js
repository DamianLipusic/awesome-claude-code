 ...
 * rolls into the empire with performers, acrobats, and exotic animals.
 * The player has 80 seconds to decide.
 *
 * Choices:
 *   🎪 Welcome Show       — pay 30 gold → +15 morale, +10 prestige
 *   🤹 Recruit Performers — pay 50 gold → +25 morale, +0.2 food/s for 2 min (settlement allure)
...
export const RECRUIT_FOOD_RATE      = 0.2;  // +0.2 food/s settlement allure
export const RECRUIT_DURATION_TICKS = 2 * 60 * TICKS_PER_SECOND; // 2 min
...
export function circusTick() {
  ...
  if (c.active) {
    if (state.tick >= c.active.expiresAt) {
      c.active        = null;
      c.nextSpawnTick = _nextSpawnTick();
...
export function getActiveCircus() {
  return state.circus?.active ?? null;
}
...
/**
 * Welcome Show — pay 30 gold, receive +15 morale and +10 prestige.
 */
export function circusWelcomeShow() {
  ...
  if ((state.resources.gold ?? 0) < SHOW_GOLD_COST) {
    return { ok: false, reason: `Need ${SHOW_GOLD_COST} gold.` };
  }
  ...
  c.active        = null;
  c.nextSpawnTick = _nextSpawnTick();
  c.totalWelcomed = (c.totalWelcomed ?? 0) + 1;

  emit(Events.CIRCUS_CHANGED, { welcomed: true });
...
/**
 * Recruit Performers — pay 50 gold, gain +25 morale and +0.2 food/s for 2 minutes.
 */
export function circusRecruitPerformers() {
  const c = state.circus;
  if (!c?.active) return { ok: false, reason: 'No circus present.' };
  if (state.tick >= c.active.expiresAt) return { ok: false, reason: 'The circus has departed.' };
  if ((state.resources.gold ?? 0) < RECRUIT_GOLD_COST) {
    return { ok: false, reason: `Need ${RECRUIT_GOLD_COST} gold.` };
  }
  ...
  if (state.randomEvents) {
    ...
    state.randomEvents.activeModifiers.push({
      id:        'circusRecruit',
      resource:  'food',
      rateMult:  1 + (RECRUIT_FOOD_RATE / Math.max(0.001, (state.rates?.food ?? 1))),
      expiresAt: state.tick + RECRUIT_DURATION_TICKS,
    });
  }

  c.active        = null;
  c.nextSpawnTick = _nextSpawnTick();
  c.totalRecruited = (c.totalRecruited ?? 0) + 1;

  emit(Events.CIRCUS_CHANGED, { recruited: true });
  emit(Events.RESOURCE_CHANGED, {});
...
/**
 * Dismiss the circus — no cost, no reward.
 */
