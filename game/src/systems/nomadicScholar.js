...
export const COMMISSION_IRON_RATE      = 0.1;
export const COMMISSION_DURATION_TICKS = 2 * 60 * TICKS_PER_SECOND;
export const COMMISSION_PRESTIGE       = 25;

export const PURCHASE_GOLD_COST        = 30;
export const PURCHASE_MANA_RATE        = 0.08;
export const PURCHASE_DURATION_TICKS   = 3 * 60 * TICKS_PER_SECOND;
...
export function initNomadicScholar() {
  if (!state.nomadicScholar) {
    state.nomadicScholar = { active: null, nextSpawnTick: _nextSpawnTick(), totalEncounters: 0, totalCommissioned: 0, totalPurchased: 0, totalDismissed: 0 };
  }
...
export function nomadicScholarTick() {
  ...
  if (ns.active) {
    if (state.tick >= ns.active.expiresAt) {
      ns.active          = null; ns.nextSpawnTick = _nextSpawnTick(); ns.totalDismissed = (ns.totalDismissed ?? 0) + 1;
...
export function getActiveNomadicScholar() { return state.nomadicScholar?.active ?? null; }
...
export function commissionScholarStudies() {
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({ id: 'scholarCommission', resource: 'iron', rateMult: 1 + (COMMISSION_IRON_RATE / Math.max(0.001, Math.abs(state.rates?.iron ?? 1))), expiresAt: state.tick + COMMISSION_DURATION_TICKS });
  }
  ns.active            = null; ns.nextSpawnTick = _nextSpawnTick(); ns.totalCommissioned = (ns.totalCommissioned ?? 0) + 1;
  emit(Events.NOMADIC_SCHOLAR_CHANGED, { commissioned: true }); emit(Events.RESOURCE_CHANGED, {});
...
export function purchaseScholarManuscripts() {
  ...
  if (state.randomEvents?.activeModifiers !== undefined) {
    ...
    state.randomEvents.activeModifiers.push({ id: 'scholarManuscripts', resource: 'mana', rateMult: 1 + (PURCHASE_MANA_RATE / Math.max(0.001, Math.abs(state.rates?.mana ?? 1))), expiresAt: state.tick + PURCHASE_DURATION_TICKS });
  }
  ns.active          = null; ns.nextSpawnTick = _nextSpawnTick(); ns.totalPurchased = (ns.totalPurchased ?? 0) + 1;
  emit(Events.NOMADIC_SCHOLAR_CHANGED, { purchased: true }); emit(Events.RESOURCE_CHANGED, {});
