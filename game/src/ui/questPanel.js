...
export function initQuestPanel() {
  ...
  panel.addEventListener('click', (e) => {
    ...
    // Desert oasis actions (T257)
    if (e.target.closest('[data-action="oasis-trade"]')) {
      const r = developOasisTradeStop();
      if (!r.ok) {
        const b = e.target.closest('[data-action="oasis-trade"]');
        if (b) { b.textContent = r.reason; setTimeout(() => render(), 1500); }
      }
    }
    if (e.target.closest('[data-action="oasis-water"]')) {
      drawOasisWater();
    }
    if (e.target.closest('[data-action="oasis-offering"]')) {
      const r = offerOasisSacrifice();
      if (!r.ok) {
        const b = e.target.closest('[data-action="oasis-offering"]');
        if (b) { b.textContent = r.reason; setTimeout(() => render(), 1500); }
      }
    }
    // Foreign dignitary actions (T258)
    if (e.target.closest('[data-action="dignitary-reception"]')) {
      const r = hostGrandReception();
      if (!r.ok) {
        const b = e.target.closest('[data-action="dignitary-reception"]');
        if (b) { b.textContent = r.reason; setTimeout(() => render(), 1500); }
      }
    }
    if (e.target.closest('[data-action="dignitary-welcome"]')) {
      offerModestWelcome();
    }
    if (e.target.closest('[data-action="dignitary-gifts"]')) {
      const r = presentDignitaryGifts();
      if (!r.ok) {
        const b = e.target.closest('[data-action="dignitary-gifts"]');
        if (b) { b.textContent = r.reason; setTimeout(() => render(), 1500); }
      }
    }
...
// ── Desert Oasis section (T257) ──────────────────────────────────────────

function _desertOasisSection() {
  const oasis = getActiveDesertOasis();
  if (!oasis) return '';

  const secs   = getDesertOasisSecsLeft();
  const urgent = secs < 20;

  const canAffordTrade    = (state.resources?.gold ?? 0) >= OASIS_TRADE_GOLD;
  const canAffordOffering = (state.resources?.mana ?? 0) >= OASIS_MANA_COST;
  const tradeClass    = canAffordTrade    ? 'btn btn--oasis-trade'    : 'btn btn--oasis-trade btn--disabled';
  const offeringClass = canAffordOffering ? 'btn btn--oasis-offering' : 'btn btn--oasis-offering btn--disabled';

  return `
    <div class="oasis-section oasis-section--active">
      <div class="oasis-header">
        <span class="oasis-icon">🌵</span>
        <span class="oasis-title">Desert Oasis Discovered</span>
        <span class="oasis-timer${urgent ? ' oasis-timer--urgent' : ''}">${secs}s</span>
      </div>
      <div class="oasis-desc">Scouts report a hidden oasis on the frontier — fresh water, shade, and opportunity await the empire. Respond within 80 seconds.</div>
      <div class="oasis-actions">
        <button class="${tradeClass}" data-action="oasis-trade"
                title="${canAffordTrade ? `Develop Trade Stop (${OASIS_TRADE_GOLD}💰)` : `Need ${OASIS_TRADE_GOLD} gold`}">
          🏪 Develop Trade Stop
          <span class="oasis-cost">${OASIS_TRADE_GOLD}💰 · +${OASIS_TRADE_PRESTIGE} prestige, +${OASIS_TRADE_RATE} gold/s for 3 min</span>
        </button>
        <button class="btn btn--oasis-water" data-action="oasis-water">
          💧 Draw Water
          <span class="oasis-cost">Free · +${OASIS_WATER_FOOD} food, +${OASIS_WATER_MORALE} morale</span>
        </button>
        <button class="${offeringClass}" data-action="oasis-offering"
                title="${canAffordOffering ? `Sacred Offering (${OASIS_MANA_COST}✨)` : `Need ${OASIS_MANA_COST} mana`}">
          🌟 Sacred Offering
          <span class="oasis-cost">${OASIS_MANA_COST}✨ · +${OASIS_OFFERING_PRESTIGE} prestige, +${OASIS_FOOD_RATE} food/s for 2 min</span>
        </button>
      </div>
    </div>`;
}

// ── Foreign Dignitary section (T258) ─────────────────────────────────────

function _foreignDignitarySection() {
  const dignitary = getActiveForeignDignitary();
  if (!dignitary) return '';

  const secs   = getForeignDignitarySecsLeft();
  const urgent = secs < 20;

  const canAffordReception = (state.resources?.gold ?? 0) >= RECEPTION_GOLD_COST;
  const canAffordGifts     = (state.resources?.food ?? 0) >= GIFT_FOOD_COST && (state.resources?.iron ?? 0) >= GIFT_IRON_COST;
  const receptionClass = canAffordReception ? 'btn btn--dignitary-reception' : 'btn btn--dignitary-reception btn--disabled';
  const giftsClass     = canAffordGifts     ? 'btn btn--dignitary-gifts'     : 'btn btn--dignitary-gifts btn--disabled';

  return `
    <div class="dignitary-section dignitary-section--active">
      <div class="dignitary-header">
        <span class="dignitary-icon">🎭</span>
        <span class="dignitary-title">Foreign Dignitary Arrives</span>
        <span class="dignitary-timer${urgent ? ' dignitary-timer--urgent' : ''}">${secs}s</span>
      </div>
      <div class="dignitary-desc">A high dignitary from a distant land arrives at the imperial court seeking an audience. How the empire receives them will shape your reputation beyond the borders. Respond within 85 seconds.</div>
      <div class="dignitary-actions">
        <button class="${receptionClass}" data-action="dignitary-reception"
                title="${canAffordReception ? `Grand Reception (${RECEPTION_GOLD_COST}💰)` : `Need ${RECEPTION_GOLD_COST} gold`}">
          🎭 Grand Reception
          <span class="dignitary-cost">${RECEPTION_GOLD_COST}💰 · +${RECEPTION_PRESTIGE_REWARD} prestige, +${RECEPTION_MORALE_REWARD} morale, +${RECEPTION_GOLD_RATE} gold/s for 4 min</span>
        </button>
        <button class="btn btn--dignitary-welcome" data-action="dignitary-welcome">
          🤝 Modest Welcome
          <span class="dignitary-cost">Free · +${WELCOME_PRESTIGE_REWARD} prestige, +${WELCOME_MORALE_REWARD} morale</span>
        </button>
        <button class="${giftsClass}" data-action="dignitary-gifts"
                title="${canAffordGifts ? `Present Gifts (${GIFT_FOOD_COST}🌾 + ${GIFT_IRON_COST}⚙️)` : `Need ${GIFT_FOOD_COST} food + ${GIFT_IRON_COST} iron`}">
          🎁 Present Gifts
          <span class="dignitary-cost">${GIFT_FOOD_COST}🌾 + ${GIFT_IRON_COST}⚙️ · +${GIFT_PRESTIGE_REWARD} prestige, +${GIFT_MORALE_REWARD} morale</span>
        </button>
      </div>
    </div>`;
}
